...
if(nrow(step1)>1){
    ...
    if(length(which(step2_ds>1))==0){
        ...
        step2_out <- data.frame()
        for(ds in names(step2_ds)[step2_ds>1]){
            tmp.sig <- step1[step1$ds==ds,] #tmp.sig is a df that contain significant rows per ds 
            ...
            tmp$Marginal.P <- tmp.sig$P[match(tmp$VARIABLE, tmp.sig$VARIABLE)] #get the marginal P from step 1
